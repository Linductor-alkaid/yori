...
- [x] `M3-01` 实现 `NvmlGpuProvider`：`dlopen` 绑定与显式 `load()`、设备发现与
...
- [x] `M3-02` 实现 `GpuManager`（`EXEC-05`/`EXEC-09`）：start 首次同步观测建基线 +
...
- [x] `M3-03` 交付 stub NVML 测试库与适配器测试：可控共享库注入设备表/遥测/逐
...
- [x] `M3-04` 交付 `GpuManager` 与调度集成测试：六场景（正常完成、任务异常=
...
- [x] `M3-05` 同步文档与验证记录：设计第 7 节适配器语义、威胁模型 NVML 观测面、
...
- [x] 适配器测试通过（stub 驱动）：load 错误映射（库缺失/驱动未装载→
...
- [x] `GpuManager` 测试通过：start 首次发布与基线；周期 tick 发布快照；观测
...
### 2026-09-08：M3-01～M3-05 PR CI 收尾

- 范围：PR [#4](https://github.com/Linductor-alkaid/yori/pull/4)，提交 `3e0d73e`～
  `8b289f5`（M3 计划、NVML 适配器、GpuManager、stub 与三项测试、GPU 探测升级、
  设计/威胁模型/总计划同步、clang-tidy 门禁修复）。
- 首轮 CI run [34249837061](https://github.com/Linductor-alkaid/yori/actions/runs/34249837061)
  暴露 6 处 clang-tidy error（两处枚举基类型过宽、load 错误映射 switch 分支
  克隆、`std::move` 常量对象、stub 句柄 int-to-ptr 转换），逐项修复
  （`8b289f5`，枚举显式 `std::uint8_t` 基类型、if 链替代 switch、`next_baseline`
  去 const、句柄改用设备数组元素真实指针），未改变产品行为；本地五预设复验
  28 用例全绿。
- 最终 CI run
  [34251754105](https://github.com/Linductor-alkaid/yori/actions/runs/34251754105)
  ...
  依赖 pin 门禁全部通过。CI 环境每套 ctest 为 21 passed、7 个环境/后续里程碑
  占位用例显式 skipped（含 `m3.platform.gpu-nvml`；真实 GPU 补跑条件：
  在带 NVIDIA GPU 的 Linux 主机运行 `ctest -L gpu`）。
- 结论：`M3-01`～`M3-05` 的实现、文档与适用门禁证据完整，工作项与退出条件
